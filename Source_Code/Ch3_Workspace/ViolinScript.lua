...
local violinBow = script.Parent

-- Ensure the model has a PrimaryPart
if not violinBow.PrimaryPart then
	warn("No PrimaryPart set for Cello bow model. Please assign one.")
	return
end

-- Define the original position and movement range
local originalPosition = violinBow.PrimaryPart.Position
local targetXPosition = 91 -- Maximum X position
...
local function moveBow()
	...
	while true do
		...
		while currentTime < 1 do
			...
			violinBow:SetPrimaryPartCFrame(CFrame.new(newPosition) * violinBow.PrimaryPart.CFrame - violinBow.PrimaryPart.Position)
		end
